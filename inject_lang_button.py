import os

ROOT_DIR = "zh"   # 你的中文 HTML 根目录（比如 zh/）

ANCHOR = '</div>\n    </div>\n'  # header-title 结束位置（稳定）

INSERT_HTML = '''
<a href="/en/" hreflang="en" class="md-header__button md-icon" title="English">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
    <path d="m12.87 15.07-2.54-2.51.03-.03A17.5 17.5 0 0 0 14.07 6H17V4h-7V2H8v2H1v2h11.17C11.5 7.92 10.44 9.75 9 11.35 8.07 10.32 7.3 9.19 6.69 8h-2c.73 1.63 1.73 3.17 2.98 4.56l-5.09 5.02L4 19l5-5 3.11 3.11zM18.5 10h-2L12 22h2l1.12-3h4.75L21 22h2zm-2.62 7 1.62-4.33L19.12 17z"/>
  </svg>
</a>
'''

for root, _, files in os.walk(ROOT_DIR):
    for fname in files:
        if not fname.endswith(".html"):
            continue

        path = os.path.join(root, fname)

        with open(path, "r", encoding="utf-8") as f:
            html = f.read()

        # 防止重复插入
        if 'href="/en/" class="md-header__button md-icon"' in html:
            continue

        if ANCHOR in html:
            html = html.replace(ANCHOR, ANCHOR + INSERT_HTML, 1)

            with open(path, "w", encoding="utf-8") as f:
                f.write(html)

            print("Injected:", path)
        else:
            print("Anchor not found:", path)
