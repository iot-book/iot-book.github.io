...
本节将以切比雪夫滤波器为例，介绍高级IIR滤波器的使用方法。切比雪夫滤波器用于将一个频带与另一个频带分开。作为一种IIR滤波器，其最大的优点是计算开销低，尽管它们无法达到基于卷积的FIR滤波器的频域滤波效果，但对于大部分应用来说，其频域响应已经足够满足应用需求了。切比雪夫滤波器的计算速度通常比相同效果下基于卷积的FIR滤波器快一个数量级，这是因为它们是通过递归而不是卷积执行的。IIR滤波器的高级设计需要基于称为z变换的数学知识，本章只介绍切比雪夫等高级IIR滤波器的基本使用方法，而不涉及对数学理论的讨论和研究。
...
理解上述滤波器设计过程需要具备较深入的数学基础知识。但实际上，对于大部分信号处理的应用来说，我们并不需要知道它们背后的数学原理。在实际操作中，更多的工程师、科学家和程序员从滤波器效果考虑，只需要能构造一个基本满足需求的滤波器就够了，为什么这么设计？参数是怎么算出来的？滤波器背后的数学原理都无需深究。
